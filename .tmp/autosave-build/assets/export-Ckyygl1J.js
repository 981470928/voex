...
`):``},mt=({contentHtml:e,includeDocumentStyles:t=!0,printStyle:n=``,title:r,watermarkInlineStyle:c=``,mask:l=null,documentRef:u})=>{let d=c?`<div class="viewer-export-watermark" style="${Xe(c)}"></div>`:``,f=i(l),p=f?{...f,regions:f.regions?.filter(e=>e.pageIndex===void 0),stamps:f.stamps?.filter(e=>e.pageIndex===void 0)}:null,m=o(p),ee=a(e,f),h=t?pt(u):``,g=n?`<style data-viewer-print-style>${n}</style>`:``,te=f?`<style data-viewer-print-mask-style>${s}</style>`:``;return`<!doctype html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8" />
...
</html>`},ht=e=>dt(mt(e),e.documentRef),gt=e=>ft(mt(e),e.documentRef),U=async({source:a,mode:o=`export`,title:s,adapter:c=null,watermarkInlineStyle:l=``,mask:u=null})=>{let d={mode:o,title:s},f=c?.toHtml,p=i(u);if(f){await t(a,c);let n=await r(await f(d)),i=await e(c,d);return{contentHtml:n,includeDocumentStyles:c.includeDocumentStyles!==!1,printStyle:i,title:s,watermarkInlineStyle:l,mask:p,documentRef:a.ownerDocument}}await t(a,c);let m=a.cloneNode(!0);m.querySelectorAll(`.viewer-watermark`).forEach(e=>e.remove()),n(a,m);let ee=await e(c,d);return{contentHtml:await r(m.innerHTML),printStyle:ee,title:s,watermarkInlineStyle:l,mask:p,documentRef:a.ownerDocument}},W=async e=>ht(await U(e)),G=async e=>gt(await U(e));export{G as buildFileViewerRenderedDomDocument,W as buildFileViewerRenderedHtmlDocument};